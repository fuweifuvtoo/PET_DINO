...
class ODVGDataset(BaseDetDataset):
    ...
    def __init__(self,
                 *args,
                 ...
                 need_text: bool = True,
                 # ------------ NOTE: customized for pet_dino
                 replace_img_prefix: dict = None,
                 label_map_tag = None,
                 # ------------
                 **kwargs) -> None:
        self.dataset_mode = 'VG'
        self.need_text = need_text
        self.replace_img_prefix = replace_img_prefix
        self.label_map_tag = label_map_tag
        if label_map_file:
    ...
    def load_data_list(self) -> List[dict]:
        ...
        for data in data_list:
            ...
            # Replace image path prefix for compatibility with different data paths.
            if self.replace_img_prefix is not None:
                for img_prefix in self.replace_img_prefix:
                    if img_prefix in img_path:
                        img_path = img_path.replace(img_prefix, self.replace_img_prefix[img_prefix])
            ...
            if self.dataset_mode == 'OD':
                ...
                # Ensure need_text is True when using label_map_tag
                assert self.need_text
                data_info['label_map_tag'] = self.label_map_tag

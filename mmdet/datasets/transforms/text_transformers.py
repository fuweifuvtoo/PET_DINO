...
def check_for_positive_overflow(gt_bboxes, gt_labels, text, tokenizer,
    ...
    # NOTE: modified for PET-DINO. np.long -> np.int64.
    # return gt_bboxes[keep_box_index], np.array(keep_gt_labels, dtype=np.long), length
    return gt_bboxes[keep_box_index], np.array(keep_gt_labels, dtype=np.int64), length
...
class RandomSamplingNegPos(BaseTransform):
    ...
    def od_aug(self, results):
        ...
        # NOTE: Convert label_map_file keys from str to int when needed.
        valid_negative_indexes = [int(i) for i in valid_negative_indexes]
        ...
        # NOTE. The label remap is performed in the following step.
        ...
        # NOTE: For cross-batch visual prompts, original labels are needed.
        # Record the reverse mapping for remapping back during training.
        reverse_label_remap_dict = {v: k for k, v in label_remap_dict.items()}
        results['reverse_label_remap_dict'] = reverse_label_remap_dict
        # Pop the large label_map dict to save space
        if 'text' in results: results.pop('text')
